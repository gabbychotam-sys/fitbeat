...
// ║  HTTP request sent AFTER alert dismisses (prevents crash!) ║
// ╚════════════════════════════════════════════════════════════╝

// Server URL - Using Cloudflare proxy (solves Garmin Let's Encrypt issue!)
const FITBEAT_SERVER_URL = "https://fitbeat.it.com/api/workout";
...
class FitBeatView extends WatchUi.View {
    ...
    var mGoalDist = 5;
    var mGoalTimeMin = 1;  // Changed to 1 minute for testing!
    var mHrMode = 0;
    var mHrTarget = 0;
    var mMaxHR = 190;
    ...
    // v4.6.7: Flag to send workout AFTER alert dismisses (prevents crash!)
    var mShouldSendWorkout = false;
    
    // v4.6.10 FIX: Save workout data BEFORE reset!
    var mSavedDurationSec = 0;
    var mSavedGpsRoute = [];
    ...
    function _loadState() {
        try {
            ...
            // ═══ FORCE 1 MINUTE FOR TESTING - IGNORE STORAGE! ═══
            mGoalTimeMin = 1;
    ...
    // ═══ START TIME GOAL - Reset distance baseline for accurate tracking! ═══
    function startTimeGoal() {
        ...
        // Reset distance and steps baseline for accurate tracking
        mStartDistCm = 0;
        mStartSteps = 0;
        try {
            var info = ActivityMonitor.getInfo();
    ...
    // v4.6.9 - CRITICAL FIX: Send via App (callbacks survive view changes!)
    // ═══════════════════════════════════════════════════════════════
    function _sendWorkoutToServer() {
        ...
        // Get local time from watch (v4.6.9)
        var localTime = "";
        try {
            var ct = System.getClockTime();
            var now = Time.now();
            var today = Gregorian.info(now, Time.FORMAT_SHORT);
            localTime = today.year.toString() + "-" + 
                (today.month < 10 ? "0" : "") + today.month.toString() + "-" + 
                (today.day < 10 ? "0" : "") + today.day.toString() + "T" +
                (ct.hour < 10 ? "0" : "") + ct.hour.toString() + ":" +
                (ct.min < 10 ? "0" : "") + ct.min.toString() + ":" +
                (ct.sec < 10 ? "0" : "") + ct.sec.toString();
        } catch(e) {
            localTime = "";
        }
        ...
        // v4.6.10 FIX: Use saved GPS route if available (current route might be empty after reset)
        var routeToSend = (mSavedGpsRoute != null && mSavedGpsRoute.size() > 0) ? mSavedGpsRoute : mGpsRoute;
        
        // Add GPS route only if we have data
        if (routeToSend != null && routeToSend.size() > 0) {
            body["route_json"] = _routeToJsonStringFrom(routeToSend);
        }
        
        // v4.6.9 FIX: Send via App (callback lives in App, survives popView!)
        try {
            var app = Application.getApp();
            if (app != null && app has :sendWorkoutRequest) {
                app.sendWorkoutRequest(body);
            }
        } catch(e) {}
    ...
    // Convert GPS route array to JSON string (to avoid Garmin array bug)
    // Using toString() - Monkey C syntax!
    // v4.6.10: Added version that takes route as parameter
    function _routeToJsonStringFrom(route) {
        try {
            ...
            for (var i = 0; i < route.size(); i++) {
                ...
                result += "{\"lat\":" + lat.toString() + ",\"lon\":" + lon.toString() + "}";
            }
            result += "]";
            return result;
        } catch(e) {
    ...
    function _routeToJsonString() {
        return _routeToJsonStringFrom(mGpsRoute);
    }
    
    // Get unique user ID based on device
    function _getUserId() {
        // First check if we have a saved ID
        var storedId = Application.Storage.getValue("uniqueUserId");
        if (storedId != null && !storedId.equals("")) {
            return storedId;
        }
        
        // Try to get device's unique identifier
        var newId = null;
        try {
            var settings = System.getDeviceSettings();
            if (settings has :uniqueIdentifier && settings.uniqueIdentifier != null) {
                // Use first 8 characters of device unique ID
                var devId = settings.uniqueIdentifier;
                if (devId.length() >= 8) {
                    newId = devId.substring(0, 8);
                } else {
                    newId = devId;
                }
            }
        ...
        // Fallback: generate from part number + timer
        if (newId == null || newId.equals("")) {
            var partNum = "unknown";
            try {
                var settings = System.getDeviceSettings();
                if (settings != null && settings.partNumber != null) {
                    partNum = settings.partNumber;
                }
            } catch(e) {}
            
            var hash = partNum.hashCode();
        ...
        // Save for future use
        Application.Storage.setValue("uniqueUserId", newId);
        return newId;
    }
    ...
    function onAlertDismissed() {
        ...
        // Send pending workout AFTER alert is dismissed (prevents crash!)
        if (mShouldSendWorkout) {
            mShouldSendWorkout = false;
    ...
    function _checkDistanceAlerts() {
        ...
        if (!mDistGoalShown && goalCm > 0) {
            if (distCm >= goalCm) {
                // v4.6.10 FIX: Save workout data BEFORE alert!
                mSavedDurationSec = mElapsedWalkSec;
                mSavedGpsRoute = mGpsRoute;
                
                // Mark that we need to send workout AFTER alert dismisses
                mShouldSendWorkout = true;
