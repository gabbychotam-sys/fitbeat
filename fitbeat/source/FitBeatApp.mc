...
class GoalPickerDelegate extends WatchUi.BehaviorDelegate {
    ...
    function onTap(evt) {
        ...
        // START button - starts DISTANCE goal and smart timer
        if (startZone != null && tapY >= startZone[0] && tapY <= startZone[1] && tapX >= startZone[2] && tapX <= startZone[3]) {
            ...
            if (mMainView != null) {
                ...
                mMainView.startDistanceGoalWithTimer();  // Starts distance + smart timer!
            }
            WatchUi.popView(WatchUi.SLIDE_DOWN);
            return true;
        }
...
class AlertView extends WatchUi.View {
    ...
    function onUpdate(dc) {
        ...
        if (mLine3 != null && !mLine3.equals("")) {
            // 3 lines format - evenly spaced across the watch face
