...
function WatchDisplay({ state, onZoneClick }) {
  ...
  // Time progress bar logic - EXACTLY like native code
  let timeFrac;
  if (goalSec > 0) {
    ...
    timeFrac = goalCm > 0 ? Math.min(distanceCm / goalCm, 1) : 0;
  } else {
    timeFrac = 0;
  }
    ...
    >
      ...
      >
        ...
        <div 
          ...
          style={{ width: '35px', height: '35px', backgroundColor: '#444', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '2px solid #666' }}
          onClick={onClose}
    ...
    >
      ...
      >
        <div style={{ 
          ...
          borderBottom: `22px solid ${mainColor}` 
        }} />
      </div>
      ...
      >
        <div style={{ 
          ...
          borderLeft: '18px solid transparent', 
          borderRight: '18px solid transparent', 
          borderTop: `22px solid ${mainColor}` 
        }} />
      </div>
...
function FitBeatSimulator() {
  ...
  const addDistance = (cm) => {
    setState(s => {
      ...
      // Check goal completion - 3 lines format with stars ★, DISTANCE CONTINUES!
