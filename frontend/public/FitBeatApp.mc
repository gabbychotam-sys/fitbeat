...
class FitBeatApp extends Application.AppBase {
    ...
    // ═══ NETWORK REQUEST STATE (lives in App, survives view changes!) ═══
    var mPendingBody = null;
    var mRetryCount = 0;
    ...
    function getInitialView() {
        mMainView = new FitBeatView();
        return [mMainView, new FitBeatDelegate(mMainView)];
    }
    
    // Called when AlertView is dismissed
    function onAlertDismissed() {
        if (mMainView != null) {
            mMainView.onAlertDismissed();
        }
    }
    
    // ═══════════════════════════════════════════════════════════════
    // NETWORK REQUEST FUNCTIONS (live in App to survive view changes!)
    // v4.6.9 - Fix: Callbacks in App don't get invalidated after popView
    // ═══════════════════════════════════════════════════════════════
    
    function sendWorkoutRequest(body) {
        mPendingBody = body;
        mRetryCount = 0;
        _doSendRequest();
    }
    
    function _doSendRequest() as Void {
        if (mPendingBody == null) {
            return;
        }
        ...
        Communications.makeWebRequest(
            "https://fitbeat.it.com/api/workout",
            mPendingBody,
            options,
    ...
    function _onWorkoutSent(responseCode as Lang.Number, data as Lang.Dictionary or Lang.String or Null) as Void {
        if (responseCode == 200 || responseCode == 201) {
            ...
            if (mRetryCount < 3 && mPendingBody != null) {
                var timer = new Timer.Timer();
                timer.start(method(:_doSendRequest), 2000, false);
            } else {
                // All retries failed
                mPendingBody = null;
                mRetryCount = 0;
...
class TimeGoalPickerView extends WatchUi.View {
    var mGoalMin = 1;  // Default: 1 minute (for testing)
    var mUpZone = null;
    var mDownZone = null;
    var mStartZone = null;
    var mCancelZone = null;  // X button for cancel/back
    ...
    function onUpdate(dc) {
        ...
        var btnW = w * 4 / 10;
        var btnX = (w - btnW) / 2;
        var btnY = h - btnH - h / 14;  // Lower position
    ...
    function onTap(evt) {
        ...
        if (startZone != null && tapY >= startZone[0] && tapY <= startZone[1] && tapX >= startZone[2] && tapX <= startZone[3]) {
            ...
            if (mMainView != null && mMainView.isDistGoalActive()) {
                // Show conflict message - must finish/reset distance goal first! (3 lines)
                var lang = getLang();
                var alertView = new AlertView(TR_GOAL_CONFLICT_LINE1[lang], TR_GOAL_CONFLICT_LINE2[lang], TR_GOAL_CONFLICT_DIST_L3[lang], "hr");
                WatchUi.pushView(alertView, new AlertViewDelegate(alertView), WatchUi.SLIDE_UP);
                return true;
            }
...
class AlertView extends WatchUi.View {
    ...
    function onDismissTimer() {
        ...
        if (!mDismissed) {
            ...
            if (mAnimTimer != null) { mAnimTimer.stop(); }
            WatchUi.popView(WatchUi.SLIDE_DOWN);
            // Notify main view that alert was dismissed
            _notifyDismissed();
        }
    }
    ...
    function _notifyDismissed() {
        // Get the app and notify the main view
        try {
            var app = Application.getApp();
            if (app != null && app has :onAlertDismissed) {
                app.onAlertDismissed();
            }
        } catch(e) {}
    }
    ...
    function onUpdate(dc) {
        ...
        if (mLine3 != null && !mLine3.equals("")) {
            // 3 lines format - EVENLY SPACED (15%, 45%, 75%)
            dc.drawText(w/2, (h*15)/100, fBig, mLine1, Graphics.TEXT_JUSTIFY_CENTER);
            dc.drawText(w/2, (h*45)/100, fBig, mLine2, Graphics.TEXT_JUSTIFY_CENTER);
            dc.drawText(w/2, (h*75)/100, fSmall, mLine3, Graphics.TEXT_JUSTIFY_CENTER);
        } else {
            // 2 lines format
