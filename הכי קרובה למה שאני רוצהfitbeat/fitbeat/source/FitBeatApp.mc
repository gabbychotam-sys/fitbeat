...
// NOTE: Connect IQ system fonts often do NOT include Hebrew glyphs.
// We ship bitmap fonts via resources/fonts.xml + resources/fonts/*.fnt/*.png
...
function _getHeSmallFont() {
    if (gHeSmallFont == null) {
        try {
            ...
            // Fallback to a safe built-in font
...
class FitBeatApp extends Application.AppBase {
    ...
    function onStart(state) {
        ...
        if (Application.Storage.getValue("activeGoalMode") == null) {
            Application.Storage.setValue("activeGoalMode", 0); // distance default
        }
    }
...
var TR_MINUTES = ["min", "דקות", "min", "min", "min", "分"];
...
// New full-screen messages (used by FitBeatView)
...
class GoalPickerView extends WatchUi.View {
    ...
    function onUpdate(dc) {
        ...
        // ═══ NEW LAYOUT: Number+Unit on LEFT, Arrows on RIGHT, START at BOTTOM ═══
        var arrowSize = w / 12;
        var centerY = h / 2 - h / 10;  // Center area for number and arrows
        ...
        // LEFT SIDE: Number + Unit (horizontal)
        var leftX = w / 6;
        ...
        // BOTTOM: START button (centered, always visible)
    ...
    function onTap(evt) {
        ...
        if (startZone != null && tapY >= startZone[0] && tapY <= startZone[1] && tapX >= startZone[2] && tapX <= startZone[3]) {
            ...
            if (mMainView != null) {
                ...
                // Distance goal session
                mMainView.startActivity(0); // MODE_DISTANCE
            }
...
class TimeGoalPickerView extends WatchUi.View {
    ...
    function onUpdate(dc) {
        ...
        var numW = dc.getTextWidthInPixels(numStr, numFont);
        var unitStr = "min";  // Always English for clarity
    ...
    function onTap(evt) {
        ...
        if (startZone != null && tapY >= startZone[0] && tapY <= startZone[1] && tapX >= startZone[2] && tapX <= startZone[3]) {
            ...
            if (mMainView != null) {
                ...
                // Time goal session
                mMainView.startActivity(1); // MODE_TIME
            }
            WatchUi.popView(WatchUi.SLIDE_DOWN);
            return true;
        }
...
class SettingsView extends WatchUi.View {
    ...
    function onUpdate(dc) {
        ...
        var titleFont = Graphics.FONT_SMALL;      // Settings title - always system font
        var itemFont  = Graphics.FONT_TINY;       // labels - always system font
        var valueFont = Graphics.FONT_XTINY;      // values - always system font
        var titleH = dc.getFontHeight(titleFont);
        var itemH = dc.getFontHeight(itemFont);

        // ─── ALL POSITIONS IN PERCENTAGES! ───
        var padSide = w / 10;             // Side padding = 10%
        var y = h / 10;                   // Start Y = 10%

        // Title: Settings (always in English)
        dc.setColor(color, Graphics.COLOR_TRANSPARENT);
        ...
        y += h / 40;
        // ===== Row 1: Language (always show "Language" in English for accessibility) =====
        var row1Top = y;

        var label1 = "Language";  // Always English so user can identify even if wrong language selected
        var valueStr1 = LANG_NAMES_MENU[lang] + " >";
        var valueW1 = dc.getTextWidthInPixels(valueStr1, valueFont);

        // Always LTR layout for Settings (system fonts don't support Hebrew)
        dc.setColor(Graphics.COLOR_WHITE, Graphics.COLOR_TRANSPARENT);
        dc.drawText(padSide, y, itemFont, label1, Graphics.TEXT_JUSTIFY_LEFT);
        ...
        // ===== Row 2: Name (always English label) =====
        var row2Top = y;
        ...
        // ===== Row 3: Color (with dot, always English) =====
        var row3Top = y;

        var dotR = h / 45;
        var colorName = COLOR_NAMES[colorIdx][0];  // Always use English color names
        ...
        // Extra spacing to avoid Save overlapping the Color row on real device fonts
        y += h / 10;

        // ===== Save button =====
        var btnW = w * 4 / 10;            // Button width = 40%
        var btnH = h / 8;
        var btnX = (w - btnW) / 2;        // Centered
        var btnY = y;
...
// ║  With colored border around screen                        ║
// ╚════════════════════════════════════════════════════════════╝
...
class AlertView extends WatchUi.View {
    ...
    // line3 is optional (pass it only if you want a 3-line alert)
...
class LanguageMenu extends WatchUi.Menu2 {
    function initialize() {
        // Menu2 uses system font (may not support Hebrew). Keep menu text ASCII-safe.
...
// ║  Fixes: Chinese keyboard + looks like real buttons          ║
// ╚════════════════════════════════════════════════════════════╝

class NameEntryView extends WatchUi.View {
    var mName = "";

    // keyboard rows (array of arrays)
    var mRows = null;

    // tap zones: each item is [y1, y2, x1, x2, ch]
    var mKeyZones = null;
    ...
    function onUpdate(dc) {
        ...
        // ═══ FINAL LAYOUT - NO GREEN BUTTON, ✓ INSIDE FIELD ═══
        
        // 1. Text field with ✓ inside (lower position, not touching edges)
        var fieldY = h / 8;
        var fieldH = h / 9;
        var fieldW = w * 6 / 10;
        ...
        // 2. Keyboard - spread across ENTIRE circle
        var totalRows = mRows.size() + 1;  // +1 for bottom controls
        ...
        for (var r = 0; r < mRows.size(); r++) {
            ...
            // Calculate width based on circle equation
        ...
        // 3. Bottom row: space (smaller) and backspace - follows circle curve
        ...
        // Space is smaller, backspace on right
...
class NameEntryDelegate extends WatchUi.BehaviorDelegate {
    ...
    function onTap(evt) {
        ...
        // Cancel (bottom ✓ button - also confirms)
